...
public class AlbumController {
    ...
    public ResponseEntity<?> search(@RequestParam(defaultValue = "0") @PositiveOrZero Integer page,
                                     ...
                                     @RequestBody SearchAlbumRequest request) {
        Page<AlbumResponse> result = albumService.findAll(PageRequest.of(page, size), request);
    ...
    public ResponseEntity<?> detail(@PathVariable("id") Integer id) {
        ApiResponse apiResponse = ApiResponse.builder()
                .code(200)
                .data(mapper.valueToTree(albumService.detail(id)))
                .build();
        return ResponseEntity.ok().body(apiResponse);
    }
